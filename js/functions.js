    ...
    $('.slide_outer').each(function () {
        ...
        if (ul_width > content_width) {
            ...
            if (offset >= 0) {
                ...
                clearInterval(menu_slide_interval);
            }
            else if (offset < 0) {
...
/* раскрытие легенлы на карте */
function mapLegend() {
	var $legend = $('.map-legend'),
		$btn = $legend.find('.ml-btn');
...
function menu_slide() {
	...
	$('.slide_right, .slide_left').mousedown(function () {
		...
		var elWidth = $('.news_list > li').outerWidth();
		...
		clearInterval(menu_slide_interval);
		menu_controls();
		_inner.animate({ 'left': slide_step }, 100, 'swing', menu_controls);
		menu_slide_interval = setInterval(function () {
			menu_controls();
		}, 1);
	})
	...
	$(function()
	{
		...
		$(".main_image_block div.vote").live("click", function(){
			$.ajax({
				url : "form/modal_form",
				dataType : "html",
				type : "GET",
				data:{"id": $(this).attr("id"), 'is_vote': 1},
				async : true,
		...
		$(".main_image_block div.result").live("click", function(){
			$.ajax({
				url : "form/modal_form_result",
				dataType : "html",
				type : "GET",
...
function popupActions()
{
	...
	{
	if (typeof($("#is_vote").val())=="undefined"){
		popup('all', 'close');
	}	
	});
...
$(window).load(function () {
	...
	var width = $('.slide_outer').width() / 4 - 40;
	$('.news_list li').css('min-width', width);
})
